...
def getSymbols():
    ...
    curl.setopt(
        curl.URL, 'https://cloud.iexapis.com/stable/ref-data/symbols?token=pk_dc1d04cb0f2c4bc5b81af61256b2fd47')
...
def calcGains():
    ...
    for record in portfolio:
        ...
        curl.setopt(curl.URL, 'https://cloud.iexapis.com/stable/stock/' +
                    symbol + '/quote?token=pk_dc1d04cb0f2c4bc5b81af61256b2fd47')
        curl.setopt(curl.WRITEFUNCTION, response_buffer.write)
        curl.perform()
        curl.close()
...
def makePlot(resource):
    ...
    curl.setopt(curl.URL, 'https://cloud.iexapis.com/stable/stock/' + symbol +
                '/chart/ytd?chartCloseOnly=true&token=pk_dc1d04cb0f2c4bc5b81af61256b2fd47')
    curl.setopt(curl.WRITEFUNCTION, response_buffer.write)
    curl.perform()
    curl.close()
